/**
 * MobileWeb(移动 web) 通用功能助手，包含常用的 UA(用户代理) 判断、页面适配、search（搜索） 参数转 键值对。
 * 该 JS 应在 head 中尽可能早的引入，减少重绘。
 *
 * fixScreen（固定筛） 方法根据两种情况适配，该方法自动执行。
 ...
 *      2. REM: 不用写 meta 标签，该方法根据 dpr（物理分辨率） 自动生成，并在 html 标签中加上 data-dpr 和 font-size 两个属性值。
